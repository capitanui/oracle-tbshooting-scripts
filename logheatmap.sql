...
column sysdate new_value date; 
select sysdate from dual;


spool c:\Workarea\Scripts\Spool\Daily_Monitor\LogHeatMap_&date..html
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('<H1>Archived Log Heat Map - '||myDBName||'  - Past days </H1>');
  ...
  LOOP
    DBMS_OUTPUT.PUT_LINE('<TR>');
    DBMS_OUTPUT.PUT_LINE('<TD style="font-family: monospace; font-weight: bold; background-color:#DEDEDE">'||TO_CHAR(cur.Day,'DD-MON-YYYY Day')||'<EM></TD>');
